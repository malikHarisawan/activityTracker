...
#             # If there's no window name, skip processing and return
#             return
...
import os
from collections import defaultdict
from graphold import plot_chart
from PyQt6.QtCore import QTimer, QEventLoop, QDate
from PyQt6.QtWidgets import (
    QApplication,
    QHBoxLayout,
...
# Import the categorization function and the show_categorized_graph function
...
class ActiveWindowMonitor(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.date_edit.setCalendarPopup(True)
        self.date_edit.setDate(QDate.currentDate())
        self.categorized_graph_button = QPushButton("Show Categorized Graph", self)
        self.go_back_button = QPushButton("Go Back to Daily Graph", self)

        button_layout.addWidget(self.decrement_button)
        button_layout.addWidget(self.date_edit)
        button_layout.addWidget(self.increment_button)
        button_layout.addWidget(self.categorized_graph_button)
        ...
        self.decrement_button.clicked.connect(self.decrement_date)
        self.date_edit.dateChanged.connect(self.update_label)
        self.categorized_graph_button.clicked.connect(self.show_categorized_graph)
        self.go_back_button.clicked.connect(self.go_back_to_daily_graph)
    ...
    def go_back_to_daily_graph(self):
        current_date = self.date_edit.date().toString("yyyy-MM-dd")
        self.figure = plot_chart(current_date)  # Reset to the daily graph
        self.canvas.figure = self.figure
        self.canvas.draw()

    def show_categorized_graph(self):
        current_date = self.date_edit.date().toString("yyyy-MM-dd")
        if current_date not in self.time_spent:
            QMessageBox.information(self, "Info", "No data available for this date.")
            return

        try:
            fig = show_categorized_graph(self.time_spent, current_date)
        except ValueError:
            QMessageBox.information(self, "Error", "No data available for this date.")
            return
